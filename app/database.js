...
function initBookTable() {
    db.run("CREATE TABLE Books (title TEXT, authorID INT, publisherID INT, genre TEXT, image TEXT, price REAL, nrRatings INT, rating FLOAT)");

    let stmt = db.prepare("INSERT INTO Books VALUES (?,?,?,?,?,?,?,?)");

    // example data
    stmt.run("Harry Potter and the Prisoner", 1, 1, "Fantasy", "./images/hp.png", 14.99, 50, 4.3);
...
function initUserTable() {
    db.run("CREATE TABLE Users (firstName TEXT, lastName TEXT, email TEXT, password TEXT)");
...
function initReviews() {
    db.run("CREATE TABLE Reviews (userID INT, bookID INT, content TEXT, date DATETIME, ananymous BOOLEAN)");

    let stmt = db.prepare("INSERT INTO Reviews VALUES (?,?,?,?,?)");

    // example data
    stmt.run(0, 0, "This is a review book was good groetjes.", "2019-06-17 16:21:23", 0);
    stmt.run(1, 1, "Another review didn't like the book so I want to be ananymous", "2011-08-42 12:26:21", 1);
